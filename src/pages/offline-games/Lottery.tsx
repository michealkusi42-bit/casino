                    ...
                    <Grid size={{ xs: 12, md: 3 }} sx={{ order: { xs: 2, md: 1 }, bgcolor: '#213743', py: { xs: 3, md: 5 }, px: { xs: 2, md: 3 } }}>
                        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
                            ...
                            <Typography variant="body2" sx={{ color: 'rgb(148 163 184)' }}>
                                Selected: {selectedNumbers.join(', ') || 'None'} ({selectedNumbers.length}/5)
                            </Typography>
                            <Typography variant="caption" sx={{ color: '#aaa' }}>
                                Match 3 = 2x | Match 4 = 10x | Match 5 = 100x
                            </Typography>
                        </Box>
                        <Button fullWidth variant="contained" disabled={isBetStarted || selectedNumbers.length !== 5} onClick={handleBet}
                            sx={{ mt: 4, py: 1.5, fontWeight: 700, backgroundColor: '#ffd700', color: '#000', '&:hover': { backgroundColor: '#ffed4a' }, borderRadius: 2 }}>
