...
/**
 * Get the list of horses and their odds for the Racing game
 * @returns Array of horses with id, name, and odds
 */
export const getRaceHorses = async () => {
    const res = await axios.get('/api/offline-game/racing/horses');
    return res.data;
};
 ...
 * Play Racing game
 * @param betAmount - Amount to bet
 * @param horseId - Id of the chosen horse
 * @returns Race result (won, finishOrder, multiplier, payout, newBalance)
 */
export const playRacing = async (betAmount: number, horseId: string) => {
    const res = await axios.post('/api/offline-game/racing/play', { betAmount, horseId });
    return res.data;
};
